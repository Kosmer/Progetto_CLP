    ...
    public Void visitInstruction(SVMParser.InstructionContext ctx) { 
    	switch (ctx.getStart().getType()) {
			...
			case SVMLexer.BRANCHLESS:
	            ...
	            i = i+1 ;
                break;
			case SVMLexer.BRANCHGREATER:
				code[i] = new AssemblyClass(SVMParser.BRANCHGREATER, ctx.REG(0).toString(), ctx.REG(1).toString(), ctx.LABEL().toString());
				i = i+1 ;
	            labelRef.put(i, (ctx.LABEL() != null ? ctx.LABEL().toString() : null));
	            i = i+1 ;
                break;
			case SVMLexer.BRANCHGREATEQ:
				code[i] = new AssemblyClass(SVMParser.BRANCHGREATEQ, ctx.REG(0).toString(), ctx.REG(1).toString(), ctx.LABEL().toString());
				i = i+1 ;
	            labelRef.put(i, (ctx.LABEL() != null ? ctx.LABEL().toString() : null));
	            i = i+1 ;
                break;
